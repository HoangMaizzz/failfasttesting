...
BENCHMARK_VERSION = "one_stage_v2_gain_calibration_validation_v3"
...
def summarize_events(group):
    ...
    return {
        ...
        "mean_gain_uncertainty": pd.to_numeric(
            group["extension_gain_uncertainty"], errors="coerce"
        ).mean(),
        "mean_gain_ucb_bonus": pd.to_numeric(
            group["extension_gain_ucb_bonus"], errors="coerce"
        ).mean(),
...
def write_manifest(args, output_dir):
    ...
    manifest = {
        ...
        "raw_prediction": "one-stage expected extension gain from prefix and extension hazards",
        "calibrated_prediction": "raw gain with Bayesian residual correction and UCB optimism",
