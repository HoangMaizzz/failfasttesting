...
from run_u1_sgd_ablation import (
    DATASETS,
    KNOWN_UNRUNNABLE_IDS,
    selected_ids as select_u1_ids,
)


ROOT = Path(__file__).resolve().parent
METHODS = ("failfast", "logistic_f2_e2", "u1_single", "u1_batch1x", "u1_batch1x_dynamic")


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--datasets", nargs="+", choices=DATASETS, default=list(DATASETS))
    parser.add_argument("--methods", nargs="+", choices=METHODS, default=["failfast", "u1_batch1x", "u1_batch1x_dynamic"])
    ...
    parser.add_argument("--dynamic_min_support", type=int, default=10)
    parser.add_argument("--dynamic_min_history", type=int, default=100)
    parser.add_argument(
        "--dynamic_history_size",
        type=int,
        default=0,
        help="0 = all resolved non-tie history",
    )
    parser.add_argument(
        "--warm_start_root",
        default=None,
        help=(
            "Optional previous raw-output root. For dynamic runs the runner loads "
            "<root>/<dataset>/u1_batch1x as the learner warm start."
        ),
    )
    parser.add_argument(
        "--dllm_dir",
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "outputs_u1_dynamic_threshold_tauD0p50_tauF0p70"
        ),
    )
...
def selected_ids(args, dataset):
    return select_u1_ids(args, dataset)
...
def command(args, dataset, method, destination):
    ...
    if method != "failfast":
        ...
        elif method == "u1_batch1x_dynamic":
            # Remove the common fixed-mode pair and replace it with the causal
            # all-history utility threshold configuration.
            mode_index = result.index("--adaptive-hindsight-logistic-threshold-mode")
            result[mode_index + 1] = "dynamic_utility"
            result.extend([
                "--adaptive-hindsight-logistic-utility-weighting", "raw_abs",
                "--adaptive-hindsight-logistic-replay-batch-size", "16",
                "--adaptive-hindsight-logistic-replay-buffer-size", "100",
                "--adaptive-hindsight-logistic-dynamic-min-support",
                str(args.dynamic_min_support),
                "--adaptive-hindsight-logistic-dynamic-min-history",
                str(args.dynamic_min_history),
                "--adaptive-hindsight-logistic-dynamic-history-size",
                str(args.dynamic_history_size),
                "--no-adaptive-hindsight-logistic-use-class-weight",
            ])
            if args.warm_start_root:
                warm_dir = Path(args.warm_start_root) / dataset / "u1_batch1x"
                result.extend([
...
def main():
    ...
    manifest = {
        ...
        "dynamic_threshold": {
            "mode": "causal all-history utility argmin",
            "min_support": args.dynamic_min_support,
            "min_history": args.dynamic_min_history,
