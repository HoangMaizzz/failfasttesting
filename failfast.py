...
def create_forward_hook(model_name, times_dict):
    """Create a forward hook to track draft/verify time based on model hidden size."""
    def forward_hook(module, input, output):
        # This hook tracks overall model forward time
        # Actual timing is done with time.perf_counter() around generate/forward calls
        pass
    return forward_hook

# Initialize time tracking dicts for each model
...
if not args.read_pickle:
    ...
    if args.mode == "dllm_ar" or args.run_dllm_sf:
        ...
        # Khiên Tied Weights (Ép trả về dictionary rỗng để chặn lỗi .keys())
        ...
        try:
            ...
            # --- 4. TẮT KHIÊN CÁCH LY (Cực kỳ quan trọng để hệ thống trở lại bình thường) ---
    ...
    for drafter_config in args.drafter_configs:
        ...
        if args.read_pickle:
            ...
            if args.mode == "verifier_ar":
                ...
                # Fix 5: Add eos_token_id and pad_token_id to prevent infinite generation
