    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if block_past_key_values is not None:
            b_k_cache, b_v_cache = None, None
            try:
                if hasattr(block_past_key_values, 'key_cache'):
                    b_k_cache = block_past_key_values.key_cache[self.layer_idx]
                    b_v_cache = block_past_key_values.value_cache[self.layer_idx]
                elif isinstance(block_past_key_values, (tuple, list)):
                    b_k_cache, b_v_cache = block_past_key_values[self.layer_idx][0], block_past_key_values[self.layer_idx][1]
            except Exception:
                pass

            if b_k_cache is None or b_v_cache is None:
                cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
                key_states, value_states = block_past_key_values.update(key_states, value_states, self.layer_idx, cache_kwargs)
            else:
                b_k_cache[:, :, replace_position:replace_position+key_states.shape[2]] = key_states
                b_v_cache[:, :, replace_position:replace_position+value_states.shape[2]] = value_states
                key_states = b_k_cache
                value_states = b_v_cache
            ...
            if update_past_key_values:
                ...
                try:
                    if hasattr(past_key_value, 'key_cache'):
                        ...
                        k_cache = past_key_value[self.layer_idx][0]
                        v_cache = past_key_value[self.layer_idx][1]
                except Exception:
                    pass
                ...
                if k_cache is not None and v_cache is not None:
                    ...
                    orig_len = past_key_value.get_seq_length() if hasattr(past_key_value, "get_seq_length") else None
                    cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
                    key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
                    if hasattr(past_key_value, 'crop') and orig_len is not None:
                        past_key_value.crop(orig_len)
...
class Fast_dLLM_QwenModel(Fast_dLLM_QwenPreTrainedModel):
    ...
    def eval_mask(self, seqlen, block_size, cache_seq_len):
        ...
        float_mask = torch.zeros(mask.shape, dtype=self.embed_tokens.weight.dtype, device=mask.device)
        float_mask = float_mask.masked_fill(~mask, torch.finfo(float_mask.dtype).min)
