...
DEFAULT_TRAJ = ROOT / 'PRECOMPUTED_ENRICHED_WITNESS' / 'BEST_TRAJECTORY_50.csv'
DEFAULT_SCHEDULE = ROOT / 'PRECOMPUTED_ENRICHED_WITNESS' / 'DETERMINISTIC_PROBE_SCHEDULE.csv'


def parse_args():
    p = argparse.ArgumentParser(description='Replay the frozen enriched-good-probe INT8/no-KV positive-control witness from zero weights.')
    ...
    p.add_argument('--min_learned_c', type=int, default=3)
...
def pooled_ms(d):
    return float(d.actual_e2e_time_excluding_transfer.sum()*1000.0 / d.output_tokens.sum())
...
def summarize_u1(case, ids, scheduled_count):
    ...
    probe = tr[src.isin(['oracle_structural_probe','oracle_floor_probe'])].copy()
    b = load_bench(case)

    action_src = dec.action_source.astype(str) if not dec.empty and 'action_source' in dec.columns else pd.Series('', index=dec.index)
    oracle_probe_actions = int(action_src.isin(['oracle_structural_probe','oracle_floor_probe']).sum())
    ...
    realized_structural = int(action_src.eq('oracle_structural_probe').sum())
    realized_floor = int(action_src.eq('oracle_floor_probe').sum())
    ...
    return {
        ...
        'sum_delta_j_learned_continue': csum(learned),
        'total_learned_benefit_ms_per_token': -csum(learned),
...
def main():
    ...
    if set(schedule.problem_id.astype(int))-set(ids):
    ...
    print('[WITNESS] INT8 verifier + FP16 compute, verifier KV OFF, drafter FP16/KV reuse OFF; threshold=0.5; frozen deterministic probe schedule')
    ...
    # Export the actual rows used in the final claim for easy audit.
    actual_tr = resolved_non_tie(load_transitions(u1_case), ids)
    if len(actual_tr):
        src_col = actual_tr.action_source.astype(str) if 'action_source' in actual_tr.columns else pd.Series('', index=actual_tr.index)
        actual_tr[src_col.eq('learned_continue')].to_csv(out/'ACTUAL_LEARNED_CONTINUES.csv', index=False)
        actual_tr[src_col.isin(['oracle_structural_probe','oracle_floor_probe'])].to_csv(out/'ACTUAL_PROBE_TRANSITIONS.csv', index=False)
    print('\n[U1 ACTUAL]')
    ...
    schedule_state_ok=bool(u1_summary['schedule_state_mismatch_count']==0)
    scheduled_count=int(len(schedule))
    encountered=int(u1_summary['scheduled_states_encountered'])
    selected=int(u1_summary['scheduled_probes_selected'])
    shadowed=int(u1_summary['scheduled_probes_shadowed_by_learned'])
    # Strong replay criterion: every frozen schedule key must be physically reached,
    # and at that state it must either execute the deterministic probe or be
    # shadowed by an autonomous learned CONTINUE.
    schedule_complete=bool(encountered==scheduled_count and selected+shadowed==scheduled_count)
    schedule_ok=bool(schedule_state_ok and schedule_complete)
    ...
    final={
        **u1_summary,
        'criteria':{
            ...
            'all_frozen_schedule_rows_must_be_encountered':True,
            'each_schedule_row_selected_or_shadowed':True,
        ...
        'pass_score':score_ok,
        'pass_schedule_state_match':schedule_state_ok,
        'pass_schedule_complete':schedule_complete,
        'frozen_schedule_rows':scheduled_count,
        'frozen_schedule_rows_encountered':encountered,
        'frozen_schedule_rows_selected':selected,
        'frozen_schedule_rows_shadowed_by_learned':shadowed,
        'PASS':passed,
    }
    ...
    verdict=(
        ...
        f"schedule_mismatches={final['schedule_state_mismatch_count']} schedule_encountered={encountered}/{scheduled_count} "
        f"selected={selected} shadowed={shadowed} max_continue_score={final['max_continue_score']:.6f}\n"
        f"probe_TP={final['probe_continue_tp']} probe_FP={final['probe_continue_fp']} "
        f"actual_probe_actions={final['oracle_probe_actions']} nominal_expected={final['nominal_expected_random_probe_count']:.3f}\n"
        "AlwaysSTOP speedup is NOT a criterion. The frozen schedule came from an oracle-guided enriched-good-probe simulation; this run tests whether that exact trajectory/schedule is physically realizable from zero weights. Individual learned-C rows may be harmful; only aggregate learned-C delta-J is constrained.\n"
    )
